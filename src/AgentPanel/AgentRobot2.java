...
public class AgentRobot2 extends Robot{
	...
	public void move(){
		double oldPi=PI.distance(field.multiTarget[field.huntedTarget].p);
		super.move();
    ...
    public void setCI() {
        if (p.distance(field.multiTarget[field.huntedTarget].p) <
                CI.distance(field.multiTarget[field.huntedTarget].p)) {
            CI.setLocation(p);
        }
    }
